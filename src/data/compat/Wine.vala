...
namespace GameHub.Data.Compat
{
	public class Wine: CompatTool
	{
		...
		public override async void run(Runnable runnable)
		{
			...
			yield wineboot(null, runnable);
			yield exec(runnable, runnable.executable, runnable.install_dir, install_postfix(), Utils.parse_args(runnable.arguments));
		}
		...
		public override async void run_action(Runnable runnable, Runnable.RunnableAction action)
		{
			...
			yield wineboot(null, runnable);
			yield exec(runnable, action.file, action.workdir, install_postfix(), Utils.parse_args(action.args));
		}
		...
		protected virtual async void exec(Runnable runnable, File file, File dir, string s="", string[]? args=null, bool parse_opts=true)
		{
			...
			yield Utils.run_thread(cmd, dir.get_path() + s, prepare_env(runnable));
		}
		...
		public async string convert_path(Runnable runnable, File path, string s)
		{
			var env = Environ.get();
			env = Environ.set_variable(env, "WINE", wine_binary.get_path());
			env = Environ.set_variable(env, "WINEDLLOVERRIDES", "mshtml=d");
			...
			var win_path = (yield Utils.run_thread({ wine_binary.get_path(), "winepath", "-w", path.get_path() + s}, runnable.install_dir.get_path(), env)).strip();
